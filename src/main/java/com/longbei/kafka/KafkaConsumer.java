...
public class KafkaConsumer {
     ...
     * @param records
     */
    @KafkaListener(topics = "${topicName.topic1}",groupId = "longbei2", containerFactory = "batchFactory")
    ...
    private void spellersToRedis(List<CanalEntry.Column> columns, boolean isDel, String tableName) {
        ...
        logger.info("isDel:" + isDel + " tableName:" + tableName + " value:" + jsonObject.toJSONString() + " key:" + redisKey);

        redisCacheManager.del("");
        redisCacheManager.del(null);
        if (isDel) {//删除
